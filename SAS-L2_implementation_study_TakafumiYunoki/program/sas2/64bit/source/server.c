...
void InitRegistration(int clntSock,unsigned char *clntIpAddr){
  ...
  //Aを受け取る
  read(clntSock,&A,sizeof(unsigned long));
  ////////////////////////
  //
  printf("A->%lu\n",A);
  //
...
//認証用関数
int Authentication(int clntSock,char *id){
  unsigned long A,C,D,E,alpha,beta,gamma;
  ...
  //alpha を受け取る
  read(clntSock,&alpha,sizeof(unsigned long));
  ////////////////////////////
  //
  printf("alpha->%lu\n",alpha);
  //
  ...
  //beta を受け取る
  read(clntSock,&beta,sizeof(unsigned long));
  ////////////////////////////
  //
  printf("beta->%lu\n",beta);
  //
  ...
  //Eを作成する
  sprintf(hash_buf,"%lu",D);
  E=crc64(hash_buf,strlen(hash_buf));
  ...
  if(C-E != 0){
    ...
    fwrite(&D,sizeof(unsigned long),1,fp);
    fclose(fp);
    
    //gammaを作成
    sprintf(hash_buf,"%lu",C);
    gamma=crc64(hash_buf,strlen(hash_buf));
...
int VernamCipher(int clntSock,char *id){
  ...
  for(i=0; i<AUTHDATA_BYTE; i++){
    vernamKey[i]=(authData & ((unsigned long)0xFF<<56-8*i))>>56-8*i;
  }
